...
let builtinLayouts = {};
let builtinLayoutLabels = {};
let defaultKeyboardLayout = 'qwerty';
...
function currentLayout() { return keyLayouts[$('keyboardLayout').value] || builtinLayouts[defaultKeyboardLayout] || [] }
function isBuiltinLayout() { return Object.hasOwn(builtinLayouts, $('keyboardLayout').value) }
function canEditBind(_cell) { return !isBuiltinLayout() }
function layoutDisplayName(name) { return builtinLayoutLabels[name] || name }
...
function applyEditorMode() { const actions = keybindMode === 'actions', binds = keybindMode === 'binds', globals = keybindMode === 'globals'; $('actionPalette').classList.toggle('collapsed', !actions); document.querySelector('.keybind-layout').classList.toggle('palette-hidden', !actions); $('actionDisplayEditor').hidden = !actions; document.querySelector('.hotkey-settings').classList.toggle('editor-hidden', !globals); document.querySelector('.layer-tabs').classList.toggle('editor-hidden', globals); $('keybindMatrix').classList.toggle('editor-hidden', globals); $('deleteBind').disabled = !binds || isBuiltinLayout(); $('deleteAssignment').disabled = !actions; $('globalsToggle').classList.toggle('mode-active', globals); $('actionsToggle').classList.toggle('mode-active', actions); $('editBinds').classList.toggle('mode-active', binds); $('actionHelp').textContent = 'Drag an action to a key, or select it and click a key. Select an action to edit its display.'; $('keybindModeHelp').textContent = binds ? (isBuiltinLayout() ? 'Built-in layouts are read-only. Use + Add Layout to create an editable copy.' : 'Hover over a cell and press a key to change that layout position.') : actions ? 'Select or drag actions. Delete Assignments removes the action from a clicked cell.' : ''; $('bindCursorMessage').classList.remove('visible'); renderActionList(); renderKeyMatrix() }
...
document.addEventListener('click', event => { if (keybindMode !== 'actions' || !selectedAction || selectedCellKey) return; const cell = event.target.closest('#keybindMatrix .key-cell[data-key]'); if (!cell) return; event.preventDefault(); event.stopImmediatePropagation(); if (assignLayerAction(keyLayer, cell.dataset.slot, selectedAction)) { selectKeyAction(''); renderKeyMatrix() } }, true);
document.addEventListener('drop', event => { if (keybindMode !== 'actions') return; const cell = event.target.closest('#keybindMatrix .key-cell[data-key]'); if (!cell) return; const target = cell.dataset.slot, action = event.dataTransfer.getData('application/x-jaw-action'); let source = null; try { source = JSON.parse(event.dataTransfer.getData('application/x-jaw-cell') || 'null') } catch { } const movedAction = action || source?.action; if (!movedAction) return; event.preventDefault(); event.stopImmediatePropagation(); if (source && source.slot === target) return; if (assignLayerAction(keyLayer, target, movedAction)) { if (source) delete keybindData[keyLayer][source.slot]; selectKeyAction(''); renderKeyMatrix() } }, true);
function keySlot(row, col) { return `P${row}${col}` }
function renderKeyMatrix() { const bindings = keybindData[keyLayer] || {}, rows = currentLayout(); $('keybindMatrix').innerHTML = rows.flatMap((row, rowIndex) => row.map((key, colIndex) => { const slot = keySlot(rowIndex, colIndex), action = bindings[slot] || '', editable = keybindMode === 'binds' && !isBuiltinLayout(), selected = selectedCellKey === slot ? 'selected' : ''; if (!key) return `<div class="key-cell key-empty ${editable ? 'bind-editable' : ''} ${selected}" data-slot="${slot}" data-row="${rowIndex}" data-col="${colIndex}"><div class="key-name">Unused</div><div class="key-action">${action ? actionDisplay(action) : '<span class="key-empty">Unassigned</span>'}</div></div>`; return `<div class="key-cell ${editable ? 'bind-editable' : ''} ${selected}" draggable="${keybindMode === 'actions' && Boolean(action)}" data-slot="${slot}" data-key="${esc(key)}" data-row="${rowIndex}" data-col="${colIndex}"><div class="key-name">${esc(key)}</div><div class="key-action">${action ? actionDisplay(action) : '<span class="key-empty">Unassigned</span>'}</div></div>` })).join(''); renderLayerWarning(); document.querySelectorAll('.key-cell').forEach(cell => { cell.onmouseenter = e => { hoveredKeyCell = cell; setCursorMessage(cell, e) }; cell.onmousemove = e => setCursorMessage(cell, e); cell.onmouseleave = () => { if (hoveredKeyCell === cell) hoveredKeyCell = null; $('bindCursorMessage').classList.remove('visible') }; cell.onclick = () => { const key = cell.dataset.key || '', slot = cell.dataset.slot; if (deleteAssignmentMode && slot) { delete bindings[slot]; selectKeyAction(''); renderKeyMatrix(); return } if (keybindMode === 'binds') { if (!canEditBind(cell)) return; if (deleteBindMode) { const row = currentLayout()[Number(cell.dataset.row)], col = Number(cell.dataset.col); row[col] = ''; if (!isBuiltinLayout()) keybindData.custom_layouts[$('keyboardLayout').value] = structuredClone(currentLayout()); selectedCellKey = ''; renderKeyMatrix() } return } if (keybindMode === 'display') { if (slot && bindings[slot]) selectKeyAction(bindings[slot], slot); return } if (keybindMode !== 'actions' || !slot) return; if (selectedAction && !selectedCellKey) { bindings[slot] = selectedAction; selectKeyAction(''); renderKeyMatrix(); return } if (bindings[slot]) { selectKeyAction(bindings[slot], slot); renderKeyMatrix() } else { selectKeyAction(''); renderKeyMatrix() } }; cell.ondragstart = e => { if (keybindMode !== 'actions') return e.preventDefault(); const slot = cell.dataset.slot, action = bindings[slot]; if (action) e.dataTransfer.setData('application/x-jaw-cell', JSON.stringify({ slot, action })) }; cell.ondragover = e => { if (keybindMode === 'actions') { e.preventDefault(); cell.classList.add('drop-over') } }; cell.ondragleave = () => cell.classList.remove('drop-over'); cell.ondrop = e => { if (keybindMode !== 'actions') return; e.preventDefault(); cell.classList.remove('drop-over'); const target = cell.dataset.slot; if (!target) return; const action = e.dataTransfer.getData('application/x-jaw-action'); if (action) bindings[target] = action; else { try { const source = JSON.parse(e.dataTransfer.getData('application/x-jaw-cell')); delete bindings[source.slot]; bindings[target] = source.action } catch { } } selectKeyAction(''); renderKeyMatrix() } }) }
function renderLayoutOptions() { const current = userData.keyboard_layout || defaultKeyboardLayout; $('keyboardLayout').innerHTML = Object.keys(keyLayouts).map(name => `<option value="${esc(name)}">${esc(layoutDisplayName(name))}</option>`).join(''); $('keyboardLayout').value = keyLayouts[current] ? current : defaultKeyboardLayout }
...
async function loadKeybinds() { keybindData = await fetch('/api/keybinds', { cache: 'no-store' }).then(r => { if (!r.ok) throw new Error(`Keybind request failed (${r.status})`); return r.json() }); builtinLayouts = structuredClone(keybindData.builtin_layouts || {}); builtinLayoutLabels = structuredClone(keybindData.builtin_layout_labels || {}); defaultKeyboardLayout = keybindData.default_layout || 'qwerty'; if (!Object.keys(builtinLayouts).length || !builtinLayouts[defaultKeyboardLayout]) throw new Error('Built-in keyboard layouts are unavailable'); keybindData.layer3 ??= {}; keybindData.action_displays ??= {}; keybindData.custom_layouts ??= {}; for (const name of Object.keys(builtinLayouts)) delete keybindData.custom_layouts[name]; keybindData.hotkey_settings ??= { toggle: 'SHIFT+SPACE', window: 'CTRL+SHIFT+F1', disable_when_minimized: false, layers: { layer2: { enabled: true, hold: true, mode: 'hold', hotkey: 'SHIFT' }, layer3: { enabled: true } } }; keyLayouts = { ...structuredClone(builtinLayouts), ...structuredClone(keybindData.custom_layouts) }; setupIconEditor(); renderLayoutOptions(); renderActionList(); renderKeyMatrix(); renderHotkeySettings(); applyEditorMode(); selectKeyAction(selectedAction); renderLayerControls(); renderTips() }
document.querySelector('[data-page="keybindPage"]').addEventListener('click', loadKeybinds); $('globalsToggle').onclick = () => setMode('globals'); $('actionsToggle').onclick = () => setMode('actions'); $('editBinds').onclick = () => setMode('binds'); $('deleteAssignment').onclick = () => { deleteAssignmentMode = !deleteAssignmentMode; deleteBindMode = false; selectedAction = ''; selectedCellKey = ''; $('deleteAssignment').classList.toggle('mode-active', deleteAssignmentMode); $('deleteBind').classList.remove('mode-active'); renderActionList(); renderKeyMatrix() }; $('deleteBind').onclick = () => { deleteBindMode = !deleteBindMode; deleteAssignmentMode = false; selectedAction = ''; selectedCellKey = ''; $('deleteBind').classList.toggle('mode-active', deleteBindMode); $('deleteAssignment').classList.remove('mode-active'); renderActionList(); renderKeyMatrix() }; $('actionSearch').oninput = renderActionList; $('disableWhenMinimized').onchange = e => keybindData.hotkey_settings.disable_when_minimized = e.target.checked; document.querySelectorAll('[data-layer-enabled]').forEach(input => input.onchange = () => { keybindData.hotkey_settings.layers[input.dataset.layerEnabled].enabled = input.checked; renderHotkeySettings() }); document.querySelectorAll('[data-key-layer]').forEach(button => button.onclick = () => { keyLayer = button.dataset.keyLayer; selectedCellKey = ''; document.querySelectorAll('[data-key-layer]').forEach(x => x.classList.toggle('active', x === button)); renderKeyMatrix() }); $('keyboardLayout').onchange = () => { selectedCellKey = ''; userData.keyboard_layout = $('keyboardLayout').value; renderKeyMatrix(); applyEditorMode() }; $('addLayout').onclick = () => { $('newLayoutName').value = ''; $('copyLayout').innerHTML = Object.keys(keyLayouts).map(name => `<option value="${esc(name)}">${esc(layoutDisplayName(name))}</option>`).join(''); $('copyLayout').value = $('keyboardLayout').value; $('layoutDialog').showModal(); requestAnimationFrame(() => $('newLayoutName').focus()) }; $('cancelLayout').onclick = () => $('layoutDialog').close(); $('createLayout').onclick = () => { const name = $('newLayoutName').value.trim(), source = $('copyLayout').value, duplicate = Object.keys(keyLayouts).some(existing => existing.toLocaleLowerCase() === name.toLocaleLowerCase()); if (!name || duplicate) { if (name) toast('Layout name is already in use'); return } keyLayouts[name] = structuredClone(keyLayouts[source]); keybindData.custom_layouts[name] = keyLayouts[name]; $('layoutDialog').close(); renderLayoutOptions(); $('keyboardLayout').value = name; renderKeyMatrix(); applyEditorMode() }; $('saveKeybinds').onclick = async () => { keybindData.custom_layouts = Object.fromEntries(Object.entries(keyLayouts).filter(([name]) => !Object.hasOwn(builtinLayouts, name))); keybindData.keyboard_layout = $('keyboardLayout').value; await api('/api/keybinds', keybindData); userData.keyboard_layout = $('keyboardLayout').value; toast('Keybinds saved') }; $('actionDisplayName').oninput = updateActionDisplay; document.addEventListener('keydown', e => { if (!$('keybindPage').classList.contains('active')) return; if (listeningHotkey) { e.preventDefault(); const value = normalizeHotkeyEvent(e), modifierOnly = ['Control', 'Alt', 'Shift', 'Meta'].includes(e.key), layerSetting = keybindData.hotkey_settings.layers[listeningHotkey]; if (modifierOnly && (!layerSetting || layerSetting.mode !== 'hold')) return; if (value) setHotkey(listeningHotkey, value); listeningHotkey = null; return } if (e.target.matches('input,textarea,select')) return; if (e.key === 'Escape') { e.preventDefault(); deleteBindMode = false; deleteAssignmentMode = false; selectedAction = ''; selectedCellKey = ''; $('deleteBind').classList.remove('mode-active'); $('deleteAssignment').classList.remove('mode-active'); renderActionList(); renderKeyMatrix(); return } if (keybindMode !== 'binds' || deleteBindMode || !hoveredKeyCell || !canEditBind(hoveredKeyCell)) return; const key = e.key === ' ' ? 'SPACE' : e.key === 'Escape' ? 'ESC' : e.key === 'Enter' ? 'ENTER' : e.key.length === 1 ? e.key.toUpperCase() : e.key.toUpperCase(); if (!key) return; e.preventDefault(); const rows = currentLayout(), row = Number(hoveredKeyCell.dataset.row), col = Number(hoveredKeyCell.dataset.col); rows[row][col] = key; if (!isBuiltinLayout()) keybindData.custom_layouts[$('keyboardLayout').value] = structuredClone(rows); renderKeyMatrix() });
...
$('saveKeybinds').addEventListener('click', () => { if (!isBuiltinLayout()) keybindData.custom_layouts[$('keyboardLayout').value] = structuredClone(currentLayout()); for (const [action, display] of Object.entries(keybindData.action_displays)) { keybindData.action_displays[action] = { label: display.label || '', icons: inferredIcons(action, display).slice(0, 2) } } }, { capture: true });
